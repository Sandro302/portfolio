#Инструкция по установке Red OS 8 и настройке окруженияДанное руководство описывает полный цикл подготовки рабочей станции: от установки ОС до настройки стека C++ / Qt / PostgreSQL / Git.1. Подготовка и установка ОСОбраз: Скачайте ISO-образ Red OS 8 (редакция «Рабочая станция»).Запись: Используйте Rufus или BalenaEtcher в режиме «DD / образ диска» (Windows) или команду dd (Linux).Внимание: Команда dd полностью стирает данные на флешке!BIOS/UEFI: Отключите Secure Boot и установите приоритет загрузки с USB.Разметка диска: Выберите «Автоматическая разметка» (для чистой установки) или «Вручную» (создайте разделы /, /home и swap).Пользователь: Обязательно отметьте пункт «Сделать администратором» (добавление в группу wheel).2. Полное обновление системыПосле первой загрузки обновите пакетную базу:Bash# Очистка кэша и построение нового
sudo dnf clean all
sudo dnf makecache

## Обновление системы
sudo dnf -y update
Важно: После обновления ядра необходима перезагрузка:Bashsudo reboot
